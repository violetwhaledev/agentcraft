...
def build_share_html(report: dict) -> str:
...
<body>
  <div class="share-stage" id="agentcraft-share">
    ...
    <div class="share-foot">
      <b>{_esc(BRAND)}</b> &middot; how you work with AI coding agents &middot;
...
def build_depth_html(report: dict) -> str:
...
def render(report: dict, out: Path, html_only: bool = False,
           page: str = "both") -> dict:
    """Render the TWO pages (SPEC section 7.1): a REDACTED shareable card
    (page 1) and a PRIVATE HUD depth dashboard (page 2). Each is HTML (always)
    + a full-height PNG (unless html_only / Chrome missing).
    ...
    Returns {"share": {...}, "depth": {...}} with the paths actually written.
    """
    share_png, depth_png = _page_paths(out)
    written: dict = {"share": None, "depth": None}

    if page in ("both", "share"):
        written["share"] = _one_page(
            lambda: build_share_html(report), share_png, html_only,
            guard=_redaction_guard)   # <-- redaction invariant enforced here
    if page in ("both", "depth"):
        written["depth"] = _one_page(
            lambda: build_depth_html(report), depth_png, html_only)
    return written
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--page", choices=["both", "share", "depth"], default="both",
                        help="which page(s) to render: both (default) | share "
                             "(page 1, redacted, shareable) | depth (page 2, "
                             "private HUD dashboard).")
